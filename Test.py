...
#Syncpoint = Content.CreateSyncpoint(Syncpoint_Name='Testing')
...
print('Get folders from syncpoints...\n')
# + json.dumps(Content.GetFoldersFromSyncpoint(All_Syncpoints[0]['Id'])) + '\n')

syncpoints = json.loads(All_Syncpoints)

for _syncpoint in syncpoints["Id"]:
  print(_syncpoint)


###json_syncpoints = json.loads(All_Syncpoints)
###for syncpoint_id in All_Syncpoints
  ###print(syncpoint_id + " ---> " + "\n")
  ###print(json.dumps(Content.GetFoldersFromSyncpoint(syncpoint_id['Id'])) + '\n'))
...
#UploadFile = Upload(Credentials, filename='%s' % Filename, full_path=Full_Path).Upload(Syncpoint[0]['Id'], Path, Storage_Endpoint_ID)
